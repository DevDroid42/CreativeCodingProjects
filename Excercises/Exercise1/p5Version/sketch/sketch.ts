...
function setupBalls(circleCount: number) {
  ...
  for (let i = 0; i < balls.length; i++) {
    ...
    balls[i] = new Ball(
      ...
      Math.round((Math.random() * windowHeight) / 10 + 20), //diameter
      new Color((i / balls.length) * 128, 255, 255) // color
    );
  }
}
...
let mic: p5.AudioIn;
let fft: p5.FFT;
let peakDetector: p5.PeakDetect;
function setup() {
...
function draw() {
  ...
  for (let i = 0; i < spectrum.length; i++) {
    vertexData[i] = createVector(
      (i / spectrum.length) * windowWidth,
      map(spectrum[i] * ((Math.log(i + 30) / 5)), 0, 255, height, 0)
    );
  }
  push();
